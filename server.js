...
if (BOT_TOKEN) {
    ...
    // Comando /start
    bot.start((ctx) => {
        const webAppUrl = process.env.WEBAPP_URL || 'https://seu-url-ngrok.ngrok-free.app';
        ctx.reply('Bem-vindo à Loja Demo!', Markup.keyboard([
            [Markup.button.webApp('🛒 Abrir Loja', webAppUrl)]
        ]).resize());
        
        // Opcional: Tenta atualizar o Menu Button do bot automaticamente
        try {
            ...
            console.error('Erro ao atualizar Menu Button:', e);
        }
    });
